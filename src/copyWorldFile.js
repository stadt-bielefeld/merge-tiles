...
function copyWorldFile(info, tile){
  ...
  outputFiles.push(info.outputDir + '/' + info.tiles.fileExt + '/x' + tile.x.from + '_y' + tile.y.from + '.' + info.tiles.worldFileExt);
  info.outputFormats.forEach((f)=>{
    if(f.fileExt !== info.tiles.fileExt){
      let outputFile = info.outputDir + '/' + f.fileExt + '/x' + tile.x.from + '_y' + tile.y.from + '.' + f.worldFileExt;
